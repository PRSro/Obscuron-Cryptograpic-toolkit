...
std::string RecipeEngine::executeSingleStep(const std::string &input, const RecipeStep &step, bool &success, std::string &error_msg) {
    ...
    try {
        if (op == "Caesar") {
            ...
            if (p.key.empty()) { success = false; error_msg = "Key cannot be empty"; return input; }
            porta(input, p.key, out);
        } else if (op == "Gronsfeld") {
            if (p.key.empty()) { success = false; error_msg = "Key cannot be empty"; return input; }
            gronsfeld(input, p.key, out, p.encrypt);
        } else if (op == "Hill") {
            int pa = p.param1 ? p.param1 : 5;
            int pb = p.param2 ? p.param2 : 7;
            int pc = p.param3 ? p.param3 : 3;
            int pd = p.param4 ? p.param4 : 11;
            bool ok;
            if (p.encrypt)
                ok = hill_encrypt(input, out, pa, pb, pc, pd);
            else
                ok = hill_decrypt(input, out, pa, pb, pc, pd);
            if (!ok) { success = false; error_msg = "Hill cipher: key matrix not invertible mod 26"; return input; }
        } else if (op == "Nihilist") {
            if (p.key.empty()) { success = false; error_msg = "Key cannot be empty"; return input; }
            if (p.encrypt) nihilist_encrypt(input, out, p.key);
            else nihilist_decrypt(input, out, p.key);
        }
            ...
            std::string real_iv = p.iv;
            if (real_iv.size() != 12) real_iv.resize(12, 0x00);
            std::string aad = p.custom_params.count("aad") ? p.custom_params.at("aad") : "";
            std::string tag;
            if (p.encrypt) {
                success = aes_gcm_encrypt(input, real_key, real_iv, aad, out, tag);
                if (success) out = to_hex((const unsigned char*)out.data(), out.size()) + " " + to_hex((const unsigned char*)tag.data(), tag.size());
            } else {
                // Decrypt: input is "ciphertext_hex tag_hex"
                size_t space = input.find(' ');
                if (space == std::string::npos) { error_msg = "AES-GCM decrypt requires format: <ciphertext_hex> <tag_hex>"; success = false; }
                else {
                    std::string ct_hex = input.substr(0, space);
                    std::string tag_hex = input.substr(space + 1);
                    std::string ct = from_hex(ct_hex);
                    std::string tag_bin = from_hex(tag_hex);
                    success = aes_gcm_decrypt(ct, real_key, real_iv, aad, tag_bin, out);
                    if (!success) error_msg = "AES-GCM decrypt failed (wrong key, tag mismatch, or corrupted data)";
                }
            }
        } else if (op == "ChaCha20") {
            std::string real_key = p.key;
            ...
            sha384_hash(input, out);
            out = from_hex(out);
        } else if (op == "SHA3-224") {
            sha3_224_hash(input, out);
            out = from_hex(out);
        } else if (op == "SHA3-256") {
            sha3_256_hash(input, out);
            out = from_hex(out);
        } else if (op == "SHA3-384") {
            sha3_384_hash(input, out);
            out = from_hex(out);
        } else if (op == "SHA3-512") {
            sha3_512_hash(input, out);
            out = from_hex(out);
        } else if (op == "SHAKE128") {
            size_t len = std::max((size_t)1, (size_t)p.param1);
            shake128_hash(input, out, len);
            out = from_hex(out);
        } else if (op == "SHAKE256") {
            size_t len = std::max((size_t)1, (size_t)p.param1);
            shake256_hash(input, out, len);
            out = from_hex(out);
        } else if (op == "Bcrypt") {
            std::string salt = p.iv;
            if (salt.size() != 16) salt.resize(16, 0);
            uint32_t rounds = (uint32_t)std::max(4, std::min(31, p.param1));
            success = bcrypt_hash(input, salt, rounds, out);
            if (!success) error_msg = "Bcrypt failed (salt must be exactly 16 bytes)";
        } else if (op == "Scrypt") {
            std::string salt = p.iv;
            uint32_t N = (uint32_t)std::max(2, p.param1);
            uint32_t r = (uint32_t)std::max(1, p.param2);
            uint32_t len = (uint32_t)std::max(8, p.param3);
            success = scrypt_hash(input, salt, N, r, 1, len, out);
            out = from_hex(out);
            if (!success) error_msg = "Scrypt KDF failed";
        } else if (op == "BLAKE2b") {
...
static const std::unordered_set<std::string>& knownOperationNames() {
    static const std::unordered_set<std::string> names = {
        "Caesar", "ROT13", "ROT47", "Atbash", "Vigenere", "Playfair",
        "Affine", "Railfence", "Columnar", "XOR (hex key)", "RC4",
        "Blowfish", "DES", "Morse", "Baconian", "Binary", "Octal",
        "Base64", "Hex", "URL Encode", "Keyword", "Substitution",
        "A1Z26", "Keyboard Shift", "Beaufort", "Autokey", "Scytale",
        "Polybius Square", "Bifid", "Trifid", "Four-Square", "Porta",
        "Gronsfeld", "Hill", "Nihilist",
        "AES-ECB", "AES-CBC", "AES-CTR", "AES-GCM", "ChaCha20",
        "Poly1305", "HMAC-SHA256", "HMAC-SHA512",
        "MD5", "SHA-1", "SHA-256", "SHA-512", "SHA-384",
        "SHA3-224", "SHA3-256", "SHA3-384", "SHA3-512",
        "SHAKE128", "SHAKE256",
        "Bcrypt", "Scrypt", "BLAKE2b", "BLAKE2s",
        "PBKDF2-SHA256", "Argon2id",
        "JWT Sign", "JWT Verify", "QR Code",
        "LSB Embed", "LSB Extract", "Leetspeak"
    };
    return names;
}
...
bool RecipeEngine::parseMacroScript(const std::string &script, RecipeModel *model, std::string &error_msg) {
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ...
        if (knownOperationNames().find(mapped_name) == knownOperationNames().end() && !m_pluginLoader) {
            error_msg = "Unknown operation: '" + mapped_name + "'";
            return false;
        }
